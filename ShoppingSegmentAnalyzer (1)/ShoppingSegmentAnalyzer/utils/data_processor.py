...
def process_clickstream_data(df):
    """
    Process raw clickstream data for analysis with custom column mapping
    """
    try:
        ...
        # Map session ID to our format
        df['session_id'] = df['session ID']

        # Generate user_id from session ID (assuming each session belongs to a unique user)
        df['user_id'] = df['session ID'].astype(str).str.split('-').str[0]

        # Combine page categories for event_type
        df['event_type'] = df.apply(
            lambda x: f"{x['page 1 (main category)']}_{x['page 2 (clothing model)']}"
            if pd.notna(x['page 2 (clothing model)'])
            else x['page 1 (main category)'],
            axis=1
        )

        # Calculate metrics
        df['purchased'] = df['order'].notna().astype(int)  # Assuming non-null order means purchase
